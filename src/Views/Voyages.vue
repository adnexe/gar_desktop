...
const voyageForm = ref<InstanceType<typeof VoyageForm> | null>(null);
        ...
        <div class="flex flex-col gap-5">
            ...
            <div class="overflow-hidden rounded-lg border bg-card p-5 shadow-sm">
                ...
                <table v-else class="w-full text-[0.95rem]">
                    ...
                    <tbody>
                        <tr v-for="v in voyages" :key="v.uuid" class="border-b last:border-0 hover:bg-muted/30">
                            ...
                            <td class="px-3 py-3"><Badge variant="outline">{{ v.statut }}</Badge></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <Dialog v-if="peutCreerVoyage" v-model:open="dialogOuvert">
            <DialogContent class="max-w-lg gap-0 overflow-hidden p-0">
                <DialogHeader class="border-b bg-background px-6 py-4">
                    <DialogTitle class="flex items-center gap-2"><Bus class="size-5" /> Créer un voyage</DialogTitle>
                </DialogHeader>
                <VoyageForm ref="voyageForm" @cree="onCree" @fermer="dialogOuvert = false" />
            </DialogContent>
        </Dialog>
    </AppSidebarLayout>
</template>
